...
-- Profiles: app users (independent of auth.users — this app uses Quran.Foundation OAuth)
...
-- Bookmarks (local mirror of QF bookmarks)
...
-- Enable RLS on all tables. The app accesses these tables exclusively via the
-- service role admin client (which bypasses RLS), so no public policies are
-- added — direct browser access is denied by default.
