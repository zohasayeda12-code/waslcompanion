// Verse counts for each of the 114 surahs (index 1..114).
...
export function nextAyahPos(surah: number, ayah: number): { surah: number; ayah: number } {
  const count = SURAH_AYAH_COUNTS[surah] ?? 0;
  if (ayah < count) return { surah, ayah: ayah + 1 };
  if (surah < 114) return { surah: surah + 1, ayah: 1 };
  return { surah, ayah }; // end of Quran
}

export function prevAyahPos(surah: number, ayah: number): { surah: number; ayah: number } {
